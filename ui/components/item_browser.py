...
class ItemButton(QPushButton):
    def __init__(self, item_code, item_name, price, currency, image_url=None, parent=None):
        ...
        # Main layout for the button to hold image and text
        ...
        if image_url:
            ...
            self.image_label.setText("📦") # Placeholder icon
        ...
        name_label.setStyleSheet("font-size: 13px; font-weight: bold; border: none; color: #111827; background: transparent;")
        
        price_str = f"{price:,.0f} {currency}".replace(',', ' ')
        price_label = QLabel(price_str)
        price_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        price_label.setStyleSheet("font-size: 12px; font-weight: bold; border: none; color: #2563eb; background: transparent;")
    ...
    def load_image(self, url):
        # Simplistic image loader (ideally this should be cached and async)
        config = load_config()
        base_url = config.get("url", "").rstrip('/')
        full_url = url if url.startswith('http') else f"{base_url}{url}"
        
        try:
            # We don't want to freeze UI, but for simplicity in this step:
            # In production, we should download images during Sync and store locally.
            # Here we just show how Pixmap works.
...
class ItemBrowser(QWidget):
    ...
    def init_ui(self):
        ...
        self.category_scroll.setStyleSheet("QScrollArea { border: none; border-right: 1px solid #e5e7eb; background: #ffffff; }")
        ...
        # --- Right Area (Search & Items Grid) ---
        right_layout = QVBoxLayout()
        right_layout.setContentsMargins(0, 10, 10, 10)

        # Search Bar
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("🔍 Tovarni izlash... (Nomi, kodi yoki shtrix-kodi) + Enter")
    ...
    def load_categories(self):
        ...
        try:
            ...
            # Fetch distinct item groups
            categories = [row.item_group for row in Item.select(Item.item_group).distinct() if row.item_group]
            
            # Add "All" button
            self.add_category_button("Barchasi", is_all=True)
            
            for cat in sorted(categories):
                self.add_category_button(cat)
    ...
    def add_category_button(self, name, is_all=False):
        ...
        if is_all:
            btn.setChecked(True) # Default selected
            
        btn.clicked.connect(lambda checked, c=name, is_a=is_all: self.on_category_clicked(btn, c, is_a))
        self.category_layout.addWidget(btn)

    def on_category_clicked(self, clicked_btn, category, is_all):
    ...
    def load_items(self, search_query=""):
        # Clear existing items in grid
        for i in reversed(range(self.items_grid.count())): 
            widget_to_remove = self.items_grid.itemAt(i).widget()
            if widget_to_remove:
                self.items_grid.removeWidget(widget_to_remove)
                widget_to_remove.setParent(None)

        try:
            db.connect(reuse_if_open=True)
            
            # Simplified query to avoid "AS" syntax error in some SQLite versions
            # We fetch Items and then manually match prices or use a simpler join
            query = Item.select()
            ...
            for item_row in query.execute():
                # Fetch price separately to avoid complex JOIN syntax errors
                ...
                btn = ItemButton(item_row.item_code, item_row.item_name, price, currency, image_url=item_row.image)
                btn.clicked.connect(lambda checked, c=item_row.item_code, n=item_row.item_name, p=price, cur=currency: 
            ...
            print(f"Error loading items grid: {e}")
        finally:
            if not db.is_closed():
                db.close()
    ...
    def select_first_item(self):
        """Adds the first item in the grid to the cart when Enter is pressed."""
        if self.items_grid.count() > 0:
            first_widget = self.items_grid.itemAt(0).widget()
            if isinstance(first_widget, ItemButton):
                self.item_selected.emit(first_widget.item_code, first_widget.item_name, first_widget.price, first_widget.currency)
