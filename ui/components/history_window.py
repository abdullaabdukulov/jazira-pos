...
class FetchHistoryWorker(QThread):
    ...
    def run(self):
        ...
        try:
            response = requests.get(endpoint, headers=self.api.get_headers(is_json=False), params=params, timeout=15)
...
class FetchDetailsWorker(QThread):
    ...
    def run(self):
        # Using frappe.client.get to fetch the full document including its items and payments
        success, doc = self.api.call_method("frappe.client.get", {
            "doctype": "POS Invoice",
...
class TransactionDetailDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.payments_area.setStyleSheet("background: #f9fafb; border: 1px dashed #d1d5db; border-radius: 8px; padding: 10px;")
    ...
    def on_details_loaded(self, success, doc):
        if not success:
        ...
        for row_idx, item in enumerate(items):
            ...
            self.table.setItem(row_idx, 2, QTableWidgetItem(f"{item.get('amount', 0):,.0f}".replace(',', ' ')))

        # 2. Fill Payments Breakdown
        ...
        for i in reversed(range(self.payments_layout.count())): 
            ...
            if item.widget(): item.widget().setParent(None)
                ...
                # Recursive clear for sub-layouts
...
class CancelOrderWorker(QThread):
    ...
    def run(self):
        ...
        success, response = self.api.call_method("ury.ury.doctype.ury_order.ury_order.cancel_order", payload)
        if success:
...
class HistoryWindow(QDialog):
    ...
    def init_ui(self):
        ...
        refresh_btn.setStyleSheet("padding: 8px 15px; background: white; border: 1px solid #d1d5db; border-radius: 6px;")
        ...
        close_btn = QPushButton("YOPISH")
        close_btn.setFixedHeight(50)
        close_btn.setStyleSheet("background-color: #ef4444; color: white; font-weight: bold; border-radius: 8px;")
        close_btn.clicked.connect(self.close)
        layout.addWidget(close_btn)
    ...
    def on_history_loaded(self, success, data):
        ...
        for row_idx, item in enumerate(data):
            ...
            self.table.setItem(row_idx, 4, QTableWidgetItem(f"{item.get('grand_total', 0):,.0f} UZS".replace(',', ' ')))
            
            if status != "Cancelled":
                cancel_btn = QPushButton("Bekor qilish")
                cancel_btn.setStyleSheet("background-color: #f97316; color: white; font-weight: bold; border-radius: 4px; padding: 5px;")
                cancel_btn.clicked.connect(lambda checked, inv=inv_name: self.confirm_cancel(inv))
    ...
    def confirm_cancel(self, invoice_id):
        reason, ok = QInputDialog.getText(self, "Bekor qilish", f"#{invoice_id} chekni bekor qilish sababi:")
        if ok and reason.strip():
            self.cancel_worker = CancelOrderWorker(invoice_id, reason.strip())
            self.cancel_worker.finished.connect(lambda s, m: (QMessageBox.information(self, "Natija", m), self.load_history()))
